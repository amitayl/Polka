...
function getDeclinedBids(loggedInUserId) {
  return new Promise((resolve, reject) => {
    ...
    DBService.dbConnect().then(db => {
      ...
      _getDeclinedBidNotifications(loggedInUserId, db).then(
        declinedBidNotifications => {
          console.log(declinedBidNotifications);
...
function _getDeclinedBidNotifications(loggedInUserId, db) {
  return new Promise((resolve, reject) => {
    const pipeline = [
      {
        $match: {
    ...
    db
      ...
      .aggregate(pipeline, (err, res) => {
        console.log('notification of declinedbid type', res);
